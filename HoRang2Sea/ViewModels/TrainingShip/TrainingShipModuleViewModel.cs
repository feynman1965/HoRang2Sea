...
namespace HoRang2Sea.ViewModels
{
    public partial class TrainingShipModuleViewModel : DocumentViewModel
    {
        ...
        // XML: TrainingShipModel.xml 파라미터명과 정확히 일치해야 함
        ...
        public TrainingShipModuleViewModel()
        {
        ...
        public TrainingShipModuleViewModel(string displayName)
        {
        ...
        public void DeleteInVisibleData()
        {
            ...
            if (BaseMWModel is TrainingShipMW TrainingShipMW)
            {
                foreach (var Tables in Database.Tables)
                {
                    foreach (var outdata in TrainingShipMW.TrainingShipMWOuts)
                    {
        ...
        {
            using (var stream = GetDataStream(@"TrainingShipModel.xml"))
            {
        ...
        {
            if (!(BaseMWModel is TrainingShipMW mw)) return;
            int step = mw.Step;
        ...
        public void OnClickCalculateButton()
        {
            if (BaseMWModel is TrainingShipMW TrainingShipMW)
            {
                ...
                if (TrainingShipMW.IsPause == true)
                {
                    Debug.WriteLine("TrainingShip: 일시정지 상태 -> 재개");
                    ...
                    try
                    {
                        if (ChartViewModel != null)
                        {
                            ...
                            Debug.WriteLine("TrainingShip: TimeChart 재개");
                        }

                        if (XYChartViewModel != null)
                        {
                            XYChartViewModel.ResetPauseState();
                            Debug.WriteLine("TrainingShip: XYChart 재개");
                        }
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine($"TrainingShip: 그래프 재개 실패: {ex.Message}");
                    }

                    TrainingShipMW.manualEvent.Set();
                    TrainingShipMW.IsPause = false;
                    MainViewModel.Status = "Calculation Resumed";
                    return;
                }

                // 이미 실행 중이면 아무것도 하지 않음
                if (TrainingShipMW.CalculateThread != null && TrainingShipMW.CalculateThread.IsAlive)
                {
                    Debug.WriteLine("TrainingShip: 이미 계산 실행 중");
                    MainViewModel.Status = "Calculation already running";
                    return;
                }

                // 새로운 계산 시작
                Debug.WriteLine("TrainingShip: 새로운 계산 시작");

                // 이전 상태 정리 (CityBus와 동일한 조건)
                if (TrainingShipMW.IsinitValue == true || (TrainingShipMW.CalculateThread != null && TrainingShipMW.CalculateThread.IsAlive))
                {
                    Debug.WriteLine("TrainingShip: 이전 상태 정리 - StopCalculation() 호출");
                    TrainingShipMW.StopCalculation();
                }
                ...
                try
                {
                    if (ChartViewModel != null)
                    {
                        ...
                        Debug.WriteLine("TrainingShip: TimeChart 초기화 완료");
                    }

                    if (XYChartViewModel != null)
                    {
                        XYChartViewModel.ClearChart();
                        Debug.WriteLine("TrainingShip: XYChart 초기화 완료");
                    }
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"TrainingShip: 그래프 초기화 실패: {ex.Message}");
                }
                ...
                TrainingShipMW.DesignLayout = DesignLayout;
                TrainingShipMW.ControlLayout = ControlLayout;
                Debug.WriteLine($"TrainingShip: Layout 설정 - Design={DesignLayout}, Control={ControlLayout}");

                TrainingShipMW.Calculate();
                StartSimMonitor();
            }
        }
        ...
        public void OnClickPauseButton()
        {
            if (BaseMWModel is TrainingShipMW TrainingShipMW)
            {
                if (TrainingShipMW.CalculateThread != null && TrainingShipMW.CalculateThread.IsAlive)
                {
                    TrainingShipMW.manualEvent.Reset();
                    TrainingShipMW.IsPause = true;
                    MainViewModel.Status = "Calculation Paused";

                    // 그래프 일시정지
                    try
                    {
                        Debug.WriteLine("TrainingShip: 그래프 일시정지");

                        if (ChartViewModel != null)
                        {
                            ChartViewModel.PauseChart();
                            Debug.WriteLine("TrainingShip: TimeChart 일시정지");
                        }

                        if (XYChartViewModel != null)
                        {
                            XYChartViewModel.PauseChart();
                            Debug.WriteLine("TrainingShip: XYChart 일시정지");
                        }
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine($"TrainingShip: 그래프 일시정지 실패: {ex.Message}");
                    }
                }
            }
        }
        public void OnClickStopButton()
        {
            if (BaseMWModel is TrainingShipMW TrainingShipMW)
            {
                if (TrainingShipMW.CalculateThread != null && TrainingShipMW.CalculateThread.IsAlive)
                {
                    Debug.WriteLine("TrainingShip: 정지");
                    ...
                    try
                    {
                        Debug.WriteLine("TrainingShip: 그래프 초기화");
        ...
        public void OnClickNextStepButton()
        {
            ...
            if (BaseMWModel is TrainingShipMW TrainingShipMW)
            {
                if (TrainingShipMW.IsPause == true) // 일시정지 상태일 경우에만 실행
                {
                    if (TrainingShipMW.CalculateThread != null && TrainingShipMW.CalculateThread.IsAlive)
                    {
                        TrainingShipMW.OneStep();
        ...
        public void OnClickSaveButton()
        {
            var dialog = new Microsoft.Win32.SaveFileDialog
            {
                ...
                DefaultExt = ".hr2v",
                FileName = "TrainingShip_Config",
                InitialDirectory = VehicleSaveData.GetDefaultDir("TrainingShip")
            };
            ...
            System.Windows.Application.Current?.Dispatcher.BeginInvoke(new Action(() =>
            {
                try
                {
                    ...
                    if (BaseMWModel is TrainingShipMW TrainingShipMW)
                    {
                        ...
                        {
                            var data = TrainingShipMW.TrainingShipMWOuts.FirstOrDefault(d => d.Name == gridItem.Name);
                            if (data != null)
                            {
