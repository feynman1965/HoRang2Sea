...
namespace HoRang2Sea.ViewModels
{
    public partial class FishingBoatModuleViewModel : DocumentViewModel
    {
        ...
        // 레이아웃 선택 다이얼로그 표시 (실제 구현)
        private void ShowLayoutSelectionDialogInternal()
        {
            var dialog = new System.Windows.Window
            {
                Title = "Select Layout - FishingBoat",
                Width = 700,
                Height = 680,
            ...
            System.Windows.Controls.Button CreateLayoutButton(string text, string description, int designValue, int controlValue)
            {
                ...
                stack.Children.Add(new System.Windows.Controls.TextBlock { Text = description, FontSize = 12, HorizontalAlignment = System.Windows.HorizontalAlignment.Center, Margin = new System.Windows.Thickness(0, 6, 0, 0) });
                button.Content = stack;

                button.Click += (s, e) =>
                {
                    // 시뮬레이션 실행 중이거나 일시정지 상태면 먼저 정지
                    if (BaseMWModel is FishingBoatMW mw && mw.CalculateThread != null && (mw.CalculateThread.IsAlive || mw.IsPause))
                    {
                        mw.StopCalculation();
                    }
